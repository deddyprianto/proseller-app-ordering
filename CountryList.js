...
export const CountryList = (props) => {
    const { withAlphaFilter, withEmoji, withFlag, withCallingCode, withCurrency, onSelect, filter, flatListProps, filterFocus, } = props;
    let { data } = props;
    const flatListRef = useRef(null);
    const [letter, setLetter] = useState('');
    const { itemHeight, backgroundColor } = useTheme();
    const defaultCountry = data.find(item => item.cca2 === 'SG');
    data = data.filter(item => item.cca2 !== 'SG')
