const ENV = {
  APP: 'CNO',
  IS_DEVELOP: true,

  API_HOST: 'https://api-cointernational.proseller-demo.com/', //DEMO

  base_url_payment: 'https://payment.proseller-demo.com/api/', //DEMO COM

  ONESIGNAL_ID: 'd0a36bd6-a0d8-4994-9045-bdb55b1dcce2',

  REGION: 'ap-southeast-1',

  ENDPOINT: 'https://xxxxxxxxxx.execute-api.ap-southeast-1.amazonaws.com/dev',

  PHONE_NUMBER_CODE: '+65',

  COUNTRY: 'Singapore',

  COMPANY_NAME: 'Co International',

  COMPANY_TYPE: 'Retail',

  COMPANY_POLICY_URL: 'https://cointernational.proseller.io',

  CURRENCY: 'SGD',

  LOCALES: 'en-US',

  SENTRY_DSN: 'https://9ed8cf3be2e24a3e884a0b15f5f054d0@sentry.io/2653559',

  PRIVATE_KEY_RSA:
    '',

  appUrl: 'maison66://app',

  appID: 'com.edgeworks.cno-demo',

  retailerInfo: '',

  netsclick_terminal_id: '',

  netsclick_retailer_id: '',

  netsclick_merchantHost: '',

  netsclick_api_key: '',

  netsclick_secret_key: '',

  environment: 'development',
};

export default ENV;
