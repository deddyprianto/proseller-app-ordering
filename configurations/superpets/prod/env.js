const ENV = {
  APP: 'Superpets',
  IS_DEVELOP: true,

  API_HOST: 'https://api-superpets.proseller.io/',

  base_url_payment: 'https://payment.proseller.io/api/',

  ONESIGNAL_ID: '2f2abd27-7365-4b61-beef-1707ac0a247e',

  REGION: 'ap-southeast-1',

  ENDPOINT: 'https://xxxxxxxxxx.execute-api.ap-southeast-1.amazonaws.com/dev',

  PHONE_NUMBER_CODE: '+65',

  COUNTRY: 'Singapore',

  COMPANY_NAME: 'Superpets',

  COMPANY_TYPE: 'Retail',

  COMPANY_POLICY_URL: 'https://superpets.proseller.io',

  CURRENCY: 'SGD',

  LOCALES: 'en-US',

  SENTRY_DSN:
    'https://950d8b27b07f52d4fb14af0c3fd2d1b3@o4505701982863360.ingest.sentry.io/4505706676748288',

  PRIVATE_KEY_RSA:
    '',

  appUrl: 'https://ordering-superpets.proseller.io/',

  appID: 'com.edgeworks.superpets-app',

  retailerInfo: '',

  netsclick_terminal_id: '',

  netsclick_retailer_id: '',

  netsclick_merchantHost: '',

  netsclick_api_key: '',

  netsclick_secret_key: '',

  environment: 'production',
};

export default ENV;
