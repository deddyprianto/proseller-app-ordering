  ...
  return React.createElement(
    ...
    React.createElement(
      ...
      React.createElement(
        ...
        React.createElement(
          CountryText,
          {allowFontScaling: false, numberOfLines: 2, ellipsizeMode: 'tail'},
          country.name,
...
const renderItem = props => ({item: country}) =>
  React.createElement(MemoCountryItem, Object.assign({}, {country, ...props}));
...
export const CountryList = props => {
  ...
  const defaultCountry = data.find(item => item.cca2 === 'SG');
  data = data.filter(item => item.cca2 !== awsConfig.COUNTRY_CODE);
  data.unshift(defaultCountry);
