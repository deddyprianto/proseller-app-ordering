...
class VirtualKeyboardCom extends Component {
  ...
  isDisabled = () => {
    const {amount} = this.state;
    const {totalPurchase} = this.props;
  ...
  render() {
    ...
    let tempAmountSVC = amountSVC;
    if (this.state.tempAmountSVC > 0) {
      tempAmountSVC = Number(this.state.tempAmountSVC);
    }
    return (
      <SafeAreaView>
        ...
        <View>
          ...
          <VirtualKeyboard
            defaultText={tempAmountSVC.toString()}
